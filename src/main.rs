...
lazy_static! {
    ...
    /// Like REVSION, this is generated by a build script.
    static ref COMPILER: Option<&'static str> = {
        let signature = include_str!(concat!(env!("OUT_DIR"), "/", "COMPILER"));
        if signature.trim().is_empty() { None } else { Some(signature) }
    };
}
...
fn log_signature() {
    if log_enabled!(Info) {
        ...
        let revision = REVISION.map(|r| format!(" (rev. {})", r))
            .unwrap_or_else(|| "".into());
        info!("{} {}{}", *NAME, version, revision);
    }
    if log_enabled!(Debug) {
        if let Some(compiler) = *COMPILER {
            debug!("Built with {}", compiler);
        }
    }
}
